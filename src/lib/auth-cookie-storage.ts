/* =============================================================
   Supabase 세션을 **도메인 공유 쿠키**에 담는 storage 어댑터.

   왜 localStorage 가 아닌가: 세션이 origin 에 갇히기 때문이다. 이 랜딩(www.wearless.kr)에서
   로그인해도 그 세션은 랜딩 것이고, 스튜디오(ai.wearless.kr)로 넘어가면 로그아웃 상태다.
   PKCE 의 code verifier 도 같은 저장소를 쓰므로, 여기서 시작한 OAuth 를 앱이 이어받는 것도
   불가능하다. `domain=.wearless.kr` 쿠키 하나로 두 문제가 같이 풀린다.

   ▶ 청크: 쿠키 한 장은 4KB 근처가 한계인데 소셜 로그인 세션(access+refresh+user metadata)은
     그보다 커질 수 있다. 그래서 `key.0`, `key.1` … 로 쪼개 담고 읽을 때 이어 붙인다.
     줄어든 청크는 반드시 지운다 — 남으면 다음 읽기가 옛 조각을 이어 붙여 JSON 이 깨진다.
   ...
   ▶ 이 파일은 **앱 레포에도 같은 내용으로 있다**(wearless_studio/src/lib/authCookieStorage.js).
